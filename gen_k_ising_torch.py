...
"""
"""
Jiajun Chen and Xinyue Chen modified on Dec 8 2024
"""
import numpy as np  
import torch
import math
...
class k_ising_torch(object):
    ...
    """
    Jiajun Chen's implementation of mean field estimation 
    """
    ...
    #     C = np.cov(s_Tnumpy)
    #     D = np.cov(s_Tnumpy[:, 1:], s_Tnumpy[:, :-1])[:N, N:]
    #     invC = np.linalg.inv(C)
    #     b = np.dot(D, invC)
    #     if not with_autocorr:
    #         np.fill_diagonal(b, 0)

    #     # Visualize Covariance Matrix (C)
    #     plt.figure(figsize=(6, 6))
    #     sns.heatmap(C, cmap="coolwarm", square=True, cbar=True, linewidths=0.2)
    #     plt.title("Covariance Matrix C")
    #     plt.show()

    #     # Visualize Time Delay Correlation Matrix (D)
    #     plt.figure(figsize=(6, 6))
    #     sns.heatmap(D, cmap="coolwarm", square=True, cbar=True, linewidths=0.2)
    #     plt.title("Time Delay Correlation Matrix D")
    #     plt.show()

    #     infJ = np.zeros((N, N))
    #     infh = np.zeros(N)
    #     A_diag = []  # To store diagonal elements of A(t)
    ...
    #             A_diag.append(gamma / a**2)  # Save a_i(t)
            
    #         else:
    #             u = minimize_scalar(self.funct, args=(delta, mi), method='brent').x
    #             a = quad(self.f2, a=-np.inf, b=np.inf, args=(u, delta))[0]
    #             A_diag.append(gamma / a**2)
    ...
    #     # Compute D from formula: D = A @ J @ C
    #     D_computed = A @ infJ @ C

    #     # Visualize Interaction Matrix (J)
    #     plt.figure(figsize=(6, 6))
    #     sns.heatmap(infJ, cmap="coolwarm", square=True, cbar=True, linewidths=0.2)
    #     plt.title("Inferred Interaction Matrix J")
    #     plt.show()

    #     # Compare Original D and Computed D
    #     plt.figure(figsize=(12, 6))
    #     plt.subplot(1, 2, 1)
    #     sns.heatmap(D, cmap="coolwarm", square=True, cbar=True, linewidths=0.2)
    #     plt.title("Original Time Delay Correlation Matrix D")

    #     plt.subplot(1, 2, 2)
    #     sns.heatmap(D_computed, cmap="coolwarm", square=True, cbar=True, linewidths=0.2)
    #     plt.title("Computed Time Delay Correlation Matrix D (A @ J @ C)")
    #     plt.show()
