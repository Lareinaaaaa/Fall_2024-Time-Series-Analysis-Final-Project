...
from gen_k_ising_torch import k_ising_torch
import torch
...
class beta_tv_torch(k_ising_torch):
    ...
    def log_likelihood(model, s_T, J, extfields, covariates_T=None, covcouplings=None):
        T, N = s_T.shape
        log_likelihood = 0
        for t in range(1, T):  # Start from t=1 as t=0 has no previous state
            st = s_T[t - 1]
            stp1 = s_T[t]
            log_likelihood += model.loglik_t(st, stp1, 
                                            covariates_T[t - 1] if covariates_T is not None else None,
                                            torch.tensor(J) if not isinstance(J, torch.Tensor) else J,
                                            torch.tensor(extfields) if not isinstance(extfields, torch.Tensor) else extfields,
                                            torch.tensor(covcouplings) if not isinstance(covcouplings, torch.Tensor) else covcouplings)
